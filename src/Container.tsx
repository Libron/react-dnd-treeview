...
export const Container = <T extends unknown>(props: Props): ReactElement => {
  ...
  return (
    <Component ref={ref} role="list" data-children={view.length} style={`--data-children:${view.length}`} {...rootProps} className={className}>
      {view.map((node, index) => (
        <React.Fragment key={node.id}>
          <Placeholder
            ...
            index={index}
          />
          <Node id={node.id} depth={props.depth} parentRef={ref} />
        </React.Fragment>
      ))}
      <Placeholder
        depth={props.depth}
